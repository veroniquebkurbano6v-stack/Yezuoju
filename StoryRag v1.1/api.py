...
def _load_index(language: Optional[str], mode: str, book: Optional[str], load_all_languages: bool = True) -> RagIndexer:
...
def create_app() -> FastAPI:
    ...
    def query_rag(payload: QueryRequest) -> QueryResponse:
        ...
        try:
            # 默认加载所有语言的数据，使用向量检索 + reranker 混排
            load_all = payload.language is None  # 如果未指定语言，则加载所有语言
            index = _load_index(lang, payload.mode, payload.book, load_all_languages=load_all)
