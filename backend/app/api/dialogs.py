...
def get_dialog_history(dialog_id: str):
    ...
    if isinstance(memory, dict) and "messages" in memory:
        ...
        # LangChain格式：需要从chat_memory对象中提取消息
        try:
            cm = getattr(memory, "chat_memory", None)
            if cm is not None and hasattr(cm, "messages"):
...
def chat(
    ...
    try:
        ...
        # 策略2：回退到字典格式的 append 操作
            ...
            memory["messages"].append({"role": "user", "content": request.question})
            memory["messages"].append({"role": "assistant", "content": result.get("answer", "")})
            logger.debug("使用字典 append 方式保存对话")
    except Exception as e:
        # 记忆保存失败不应该阻塞响应，仅记录日志
